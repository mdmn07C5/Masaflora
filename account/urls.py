...
from . import views
from .forms import (UserLoginForm, PWResetForm, PWResetConfirmForm)
...
urlpatterns = [
    ...
    path('profile/deactivate_confirm', TemplateView.as_view(
        template_name='account/user/deactivate_confirm.html'), name='deactivate_confirm'),
    # password reset
    path('password_reset/',
         auth_views.PasswordResetView.as_view(template_name='account/user/password_reset_form.html',
                                              success_url='password_reset_email_confirm',
                                              email_template_name='account/user/password_reset_email.html',
                                              form_class=PWResetForm), name='password_reset'),
    path('password_reset_confirm/<uidb64>/<token>/', auth_views.PasswordResetConfirmView.as_view(template_name='account/user/password_reset_confirm.html',
                                                                                                 ...
                                                                                                 form_class=PWResetConfirmForm), name='password_reset_confirm'),
    path('password_reset/password_reset_email_confirm/', TemplateView.as_view(
        template_name='account/user/password_reset_status.html'), name='password_reset_done'),
    path('password_reset_complete/', TemplateView.as_view(
        template_name='account/user/password_reset_status.html'), name='password_reset_complete'),
